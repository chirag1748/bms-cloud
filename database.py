...
def fmt_display_dt(value):
    """Convert stored 'yyyy-mm-dd HH:MM' -> 'dd-mm-yyyy HH:MM' for display/export.
    Leaves the value unchanged if it doesn't match the expected pattern."""
    if not value:
        return ""
    try:
        date_part, time_part = (value.split(" ", 1) + [""])[:2]
...
def init_db():
    ...
    db.tickets.create_index([("assigned_time", DESCENDING)])
...
def search_tickets(token_no="", rc_no="", technician="",
    ...
    if from_date or to_date:
        # Tickets have no standalone 'date' field — the ticket's effective
        # date is the date portion of assigned_time ("yyyy-mm-dd HH:MM").
        # from_date/to_date arrive as "yyyy-mm-dd" (HTML date inputs), so a
        # plain string range comparison on the date portion sorts correctly.
        query["assigned_time"] = {}
        if from_date: query["assigned_time"]["$gte"] = from_date + " 00:00"
        if to_date:   query["assigned_time"]["$lte"] = to_date   + " 23:59"
    docs = db.tickets.find(query).sort("created_at", DESCENDING).limit(500)
    return [_clean(d) for d in docs]


def get_tickets_for_export(token_no="", rc_no="", technician="",
                           from_date=None, to_date=None):
    """Same filters as search_tickets but no row limit — used for Excel export."""
    db    = get_db()
    query = {}
    if token_no:   query["token_no"]   = {"$regex": token_no,   "$options": "i"}
    if rc_no:      query["rc_no"]      = {"$regex": rc_no,      "$options": "i"}
    ...
    if from_date or to_date:
        ...
        if from_date: query["assigned_time"]["$gte"] = from_date + " 00:00"
        if to_date:   query["assigned_time"]["$lte"] = to_date   + " 23:59"
    docs = db.tickets.find(query).sort("created_at", DESCENDING)
    return [_clean(d) for d in docs]
...
def get_tickets_in_range(from_date, to_date):
    ...
    docs = db.tickets.find({
        "assigned_time": {"$gte": from_date + " 00:00", "$lte": to_date + " 23:59"}
    })
    return [_clean(d) for d in docs]
